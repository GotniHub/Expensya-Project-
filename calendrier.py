        ...
        if missing:
            ...
            if cal_df.empty:
                ...
                if cal_f.empty:
                    ...
                    mat_file = st.file_uploader("➕ (optionnel) Matrice Expensya (.xlsx) pour afficher le libellé mission", type=["xlsx"], key="mat_for_cal")
                    client_to_label = {}
                    if mat_file is not None:
                        try:
                            ...
                            if {"Client (Référence)", "Modification Code Expensya"}.issubset(_mat.columns):
                                client_to_label = dict(zip(_mat["Client (Référence)"].astype(str), _mat["Modification Code Expensya"].astype(str)))
                        except Exception:
                            pass  # si la matrice n'est pas fournie/valide, on affiche simplement le code client

                    # ---------- Hover enrichi avec libellé mission ----------
                    def mission_label(client_code: str) -> str:
                        code = str(client_code)
                        if code in client_to_label and pd.notna(client_to_label[code]) and client_to_label[code].strip():
                            return f"{code} — {client_to_label[code]}"
                        return code

                    def fmt_hover(row):
                        # TTC (EUR) / Devise si dispo
                        ttc = ""
                        if "TTC (EUR)" in row and pd.notna(row["TTC (EUR)"]):
                            ttc = f" - {row['TTC (EUR)']:.2f} EUR"
                        elif "TTC" in row and pd.notna(row["TTC"]):
                            ttc = f" - {row['TTC']}"
                        return (
                            f"{row['Nom de la dépense']} ({row['Catégorie']}){ttc}"
                            f"<br>Client / Mission : {mission_label(row['Client (Référence)'])}"
                            f"<br>Date : {row['Date'].strftime('%Y-%m-%d')}"
                        )

                    events["hover"] = events.apply(fmt_hover, axis=1)
                    ...
                    # Inverser l’axe Y pour un affichage plus naturel
                    fig.update_yaxes(autorange="reversed")
                    # Remplacer le texte du hover par notre champ formaté
                    fig.update_traces(
...
# Fabriquer le hover détaillé (liste des notes de frais du jour)
# On remerge pour obtenir les lignes de ce (Utilisateur, Jour)
idx_cols = ["Utilisateur","Jour"]
day_rows = tmp[idx_cols + ["Catégorie","Nom de la dépense","Date"]]
if "TTC (EUR)" in tmp.columns:
    day_rows = day_rows.join(tmp["TTC (EUR)"])
elif "TTC" in tmp.columns:
    day_rows = day_rows.join(tmp["TTC"])

def build_hover(u, d):
    sub = day_rows[(day_rows["Utilisateur"]==u) & (day_rows["Jour"]==d)]
    lines = [to_item_row(r) for _,r in sub.iterrows()]
    # Titre
    title = f"{u} — {d.strftime('%Y-%m-%d')}"
